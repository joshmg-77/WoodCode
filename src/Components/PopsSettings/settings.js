...
import useGenerateRandomHex from '../../hooks/useGenerateRandomHex';
import DivContent from '../common/DivContent';
import FrameOptionsBackground from './FrameOptionsBackground';
...
const GlobalStyle = createGlobalStyle`
  ...
  pre[class*="language-"] {  /* override for line-number.css
    ...
    box-shadow:${({ boxShadow }) => boxShadow && `0px 0px 15px 2px #000000 `};
    box-shadow:${({ boxShadowInset, backgroundColor }) =>
      boxShadowInset &&
      `0px 0px 25px 0px ${backgroundColor} inset,0px 0px 25px 10px rgba(0,0,0,0.5) inset`}; 
  ...
  `}
   ${({ lineNumbers }) =>
     lineNumbers &&
     `textarea.prism-live{padding-left:0px !important;}
  `}
...
export default function Settings({ openItem, onBackInner }) {
  ...
  const HandleChangeInputs = (event) => {
    let value =
      ...
      event.target.name === 'box_shadow' ||
      event.target.name === 'box_shadowInset'
